...
### AWS (GitHub Actions)
The project is pre-configured for deployment to AWS (including AWS Academy Learner Labs) via GitHub Actions.
1. **ECR Repositories:** Create `shopmart-server` and `shopmart-client` in Amazon ECR.
2. **GitHub Secrets:** Add `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `AWS_SESSION_TOKEN` (for Academy) to your repository.
3. **App Runner / EC2:** Use the provided `docker-compose.prod.yml` and `deploy.sh` for easy server setup.
4. See the **[AWS Deployment Guide](.gemini/antigravity/brain/71c50dd6-cd11-4029-bc5f-37058dd9b7e6/aws_deployment_guide.md)** (generated by AI) for detailed steps.
